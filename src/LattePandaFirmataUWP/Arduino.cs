...
namespace LattePandaFirmataUWP
{
    ...
    *************************************************************/
    ...
    namespace Firmata
    {
        ...
        class Arduino
        {
            ...
            {
                string aqs = Windows.Devices.SerialCommunication.SerialDevice.GetDeviceSelector(serialPortName);

                myDevices = Windows.Devices.Enumeration.DeviceInformation.FindAllAsync(aqs, null).GetResults();

                _serialPort = SerialDevice.FromIdAsync(myDevices[0].Id).GetResults();
                // SerialPort(serialPortName, baudRate);
                _serialPort.DataBits = 8;
                _serialPort.Parity = SerialParity.None;// Parity.None;
                _serialPort.StopBits = SerialStopBitCount.One;// StopBits.One;
            ...
            public void Open()
            {
                _serialPort.IsDataTerminalReadyEnabled = true;// .DtrEnable = true;
                //_serialPort.//.Open();
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    command[1] = (byte)1;
                    _dataWriter.WriteBytes(command);//..Write(command, 0, 2);
                }
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    command[1] = (byte)1;
                    _dataWriter.WriteBytes(command);//_dataWriter.Write(command, 0, 2);
                }
                command = null;

                if (_readThread == null)
                {
                    _readThread = new Task(processInput); //new PreallocatedWorkItem(); //new Thread(processInput);
                    _readThread.Wait(); //.Start();
                }
            }
            /// <summary>
            ...
            {
                return myDevices.Select(x => x.Id).ToArray();
                // .GetPortNames();
            }
            /// <summary>
            ...
            public void pinMode(int pin, byte mode)
            {
                ...
                message[1] = (byte)(pin);
                message[2] = (byte)(mode);
                _dataWriter.WriteBytes(message);//_serialPort.Write(message, 0, 3);
                message = null;
            }
            /// <summary>
            ...
            public void wireBegin(Int16 delay)
            {
                ...
                _dataWriter.WriteBytes(message);//_serialPort.Write(message, 0, 5);
            }
            /// <summary>
            /// Write to a digital pin that has been toggled to output mode with pinMode() method.
            /// </summary>
            ...
            public void wireRequest(byte slaveAddress, Int16 slaveRegister, Int16[] data, byte mode)
            {
                ...
                _dataWriter.WriteBytes(message);//_serialPort.Write(message, 0, index + 1);
            }
            private int available()
            {
                return (int) _serialPort.BytesReceived; //.BytesToRead;
            }
            public void processInput()
            {
                while (_serialPort != null)
                {
                    ...
                    if (_dataReader.UnconsumedBufferLength > 0)
                    {
                        lock (this)
                        {
                            int inputData = _dataReader.ReadByte();
                            int command;
            ...
            private static DeviceInformationCollection myDevices;
            ...
            private Windows.System.Threading.Core.PreallocatedWorkItem _writeThread = null;
